...
namespace vje
{
	...
	QString JsonParser::decode_string ( const QString& rawTokenText )
	{
		...
		//
		// The escape table itself is json_escapes' (architecture.md section 4.6) -- moved there when the Form and Text
		// views needed to show a value in escaped notation, so the reading and the writing of an escape stay one
		// definition. A QStringView over the body keeps this hot path allocation-free on the way in; a malformed token
		// cannot arrive here, and if one somehow did, decode() stops at it and returns what it had.

		QString decoded;

		json_escapes::decode ( QStringView { rawTokenText }.sliced ( 1, rawTokenText.size () - 2 ), decoded );
