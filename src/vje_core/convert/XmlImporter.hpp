...
namespace vje
{
	...
	struct XmlElement
	{
		...
		// Did this element carry namespace information that the JSON does not represent -- a prefix on its own name or
		// on one of its attributes, or an xmlns declaration? Names reach the JSON as LOCAL names, so a prefix is
		// silently dropped; recording that here is the parser's job, because it is the only thing that saw it. Nothing
		// in the conversion reads this: it exists so the FILE-13 preview can warn about the loss (section 2.11).
	...
	public:
		...
		// The parsed XML on its own, before any strategy has looked at it. Separated from Result because the FILE-13
		// preview changes strategy far more often than it changes file: parsing is the expensive half and depends on
		// nothing the dialog can alter, so it is done once and re-converted per change (section 2.11's "instantaneous").
	...
	public:
		...
		//
		// This is exactly parse() followed by convert_tree(), and is the whole of the import pipeline's use of the
		// class. The two halves are public for the preview, which needs to hold the tree between conversions.

		static Result import_text ( const QString& xml, const IXmlImportStrategy& strategy, bool inferScalars );

		// The two halves of import_text.

		static ParseResult parse ( const QString& xml );

		static std::unique_ptr<JsonNode> convert_tree
		(
			const XmlElement&         root,
			const IXmlImportStrategy& strategy,
			bool                      inferScalars
		);
	...
	//-----------------------------------------------------------------------------------------------------------------
	// Walk a parsed tree, root first, calling visit for every element. Used by the FILE-13 preview to count the
	// constructs a strategy cannot fully represent (section 2.11); the conversion itself does not need it.
	//-----------------------------------------------------------------------------------------------------------------

	void visit_xml_elements ( const XmlElement& element, const std::function<void ( const XmlElement& )>& visit );
}
