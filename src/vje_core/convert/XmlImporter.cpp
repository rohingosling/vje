...
namespace vje
{
	...
	namespace
	{
		...
		XmlElement read_element ( QXmlStreamReader& reader )
		{
			...
			// name() is the LOCAL name -- QXmlStreamReader resolves namespaces and strips the prefix, and it reports
			// xmlns declarations through namespaceDeclarations() rather than as attributes. Both are therefore
			// invisible below this line, so whether there WAS any is recorded here and nowhere else (section 2.11).

			element.namespaced = !reader.prefix ().isEmpty () || !reader.namespaceDeclarations ().isEmpty ();
			...
			for ( const QXmlStreamAttribute& attribute : attributes )
			{
				...
				element.namespaced = element.namespaced || !attribute.prefix ().isEmpty ();
			}
	...
	XmlImporter::ParseResult XmlImporter::parse ( const QString& xml )
	{
		ParseResult result;

		result.ok = parse_root ( xml, result.root, result.error );

		return result;
	}

	std::unique_ptr<JsonNode> XmlImporter::convert_tree
	(
	...
	)
	{
		...
		std::unique_ptr<JsonNode> document = JsonNode::make_object ();

		document->append_member ( root.name, strategy.convert ( root, inferScalars ) );
	...
	{
		Result            result;
		const ParseResult parsed = parse ( xml );
		...
		result.root = convert_tree ( parsed.root, strategy, inferScalars );
		result.ok   = true;
	...
	void visit_xml_elements ( const XmlElement& element, const std::function<void ( const XmlElement& )>& visit )
	{
		...
		for ( const XmlElement& child : element.children )
		{
			visit_xml_elements ( child, visit );
		}
	}
}
