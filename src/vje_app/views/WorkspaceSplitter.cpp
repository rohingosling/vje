...
namespace vje
{
	...
	QList<int> nudged_splitter_sizes ( const QList<int>& sizes, int delta, int minimumFirst, int minimumSecond )
	{
		...
		// The room the first pane actually has, which is bounded at BOTH ends -- by its own minimum and by whatever the
		// second pane needs. Computed before the nudge so a window too narrow to satisfy both minimums cannot produce a
		// nonsensical upper bound below its lower one.

		const int lowest  = minimumFirst;
		const int highest = std::max ( minimumFirst, total - minimumSecond );

		const int nudged = std::clamp ( sizes.at ( 0 ) + delta, lowest, highest );

		return { nudged, total - nudged };
	}
