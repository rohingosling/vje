...
namespace vje
{
	...
	public:
		...
		// The CELL clipboard (EDITOR-11). Edit > Cut / Copy / Paste route here when the tree does not own the gesture and
		// no text editor has focus; a view that has a cell to act on handles it and returns true, and any other view
		// declines so the caller falls back to the node clipboard. Only the Form View's array table answers; everything
		// else keeps the default no-op.

		virtual bool cell_cut   () { return false; }
		virtual bool cell_copy  () { return false; }
		virtual bool cell_paste () { return false; }

		// Is a cell clipboard command applicable right now (the array table with a current cell)? Drives the
		// disabled-not-hidden enablement of Edit > Cut / Copy / Paste.

		virtual bool cell_clipboard_active () const { return false; }

		// What File > Print should put on paper (FILE-12): THIS view's rendering of what it is currently showing, in
		// the two shapes print_content.hpp defines.
		//
		// availableColumns is the PAGE's width in characters of the print's fixed-width font, and it is passed in
		// because a rendering laid out by counting characters has to be produced at the width it will be read at. A
		// view wrapping for its own pane and letting the page re-break the result loses the alignment the wrapping
		// existed for -- the Text View's continuation lines land under the value column when the renderer wraps them
		// and at column 0 when the page does. 0 means "unknown", and a view then leaves its content unwrapped.
		//
		// The default answers Kind::Empty, so a view that has nothing meaningful to put on paper -- and any view added
		// later, until it decides -- simply declines and the command reports that rather than printing a blank page.

		virtual PrintContent print_content ( int availableColumns ) const
		{
