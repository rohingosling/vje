...
namespace vje
{
	...
	public:
		...
		// FILE-12: the same rendering, produced at the PAGE's width rather than the pane's.
		//
		// Handing over the on-screen buffer was the obvious thing and was wrong: this renderer wraps with a hanging
		// indent under the value column, and a page narrower in characters than the pane re-breaks each of those lines
		// at column 0 -- so a printed entry alternated between aligned continuations and unaligned ones, which is the
		// defect this replaced. Re-rendering at the page's own width is what makes the name-separator margin hold on
		// paper, and it wraps whatever SET-06 says: a page has a hard physical width and no horizontal scrolling.
	...
	private:
		...
		// The viewport's width in CHARACTERS, or 0 when Wrap strings is off (SET-06). Exact rather than approximate
		// because the font is fixed-width -- see the note on the font above.

		int wrap_columns () const;
	...
	protected:
		...
		// The wrap width follows the window, so a resize can be a re-render -- filtered to an actual change in COLUMNS,
		// since a pane drag is dozens of pixel resizes and at most a few column boundaries.
	...
	private:
		...
		// The wrap width the current rendering was produced at, so a resize that does not cross a column boundary
		// costs nothing. -1 rather than 0, because 0 is a legitimate width meaning "wrapping is off".

		int renderedWrapColumns = -1;
	};
