...
namespace vje
{
		...
		, titleText  ( title )
	{
		...
		// NFR-05. The band paints its title rather than hosting a QLabel, so there is no text for the accessibility
		// framework to find on its own -- a painted string is invisible to it by construction.
	...
	void PaneHeader::set_title ( const QString& title )
	{
		...
		// The painted title and the accessible name are the same claim, so they are written together rather than left
		// to be kept in step by hand (NFR-05).
	...
	void PaneHeader::paintEvent ( QPaintEvent* event )
	{
		...
		// The band's own strong / receded pair, stretched the width of the pane (STYLE-13). It borrows a tab's LOOK,
		// not its shape -- and, since the 2026-07-24 review, not its surface function either: band_surface is
		// DECOUPLED from the selected tab, which is what lets the light strip's static field match the receded band
		// exactly while the unfocused active tab sits a step darker than both. The strong shade still coincides with
		// the focused active tab's, so the two panes keep reading as one register when either holds the keyboard.

		painter.fillRect ( band, band_surface ( palette (), paneFocused ) );
