...
//
//   IT WEARS A TAB'S LOOK WITHOUT BEING A TAB. The fill is band_surface (style/tab_surface.hpp): the band's own
//   strong / receded pair, whose strong shade coincides with the focused active tab's -- so the band sits in the same
//   visual register as the editor pane's tab strip across the splitter. Since the 2026-07-24 review the pair is
//   DECOUPLED from the selected tab's surface: the light strip's static field matches the receded band exactly, while
//   the unfocused active tab sits a step darker than both. The SHAPE is a plain full-width bar: no rounded ends, no
//   tab glyph, and a left-aligned title rather than a centred label -- it names a pane rather than selecting one.
//
...
namespace vje
{
	...
	public:
		...
		// the pane's FocusHighlight, which is the application's single answer to it. The band paints its own strong /
		// receded pair from band_surface (style/tab_surface.hpp).
