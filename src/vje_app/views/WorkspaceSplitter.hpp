...
namespace vje
{
	...
	//-----------------------------------------------------------------------------------------------------------------
	// Where the splitter lands after a keyboard nudge (NAV-06, NFR-05).
	//
	// A PURE FUNCTION over the sizes, so the whole decision -- how far, and what happens at each end -- is testable
	// without a window, and the window is left with nothing but a call. `delta` is applied to the FIRST pane: negative
	// narrows it, positive widens it.
	//
	// The clamping is the only interesting part, and it is a refusal rather than a squeeze: a nudge that would push
	// either pane under its minimum stops at that minimum instead, so holding the key down parks the splitter at the
	// edge rather than walking the other pane out of existence. A list that is not exactly two panes is returned
	// unchanged -- this workspace has two, and a silent guess about a third would be worse than doing nothing.
	//-----------------------------------------------------------------------------------------------------------------

	QList<int> nudged_splitter_sizes ( const QList<int>& sizes, int delta, int minimumFirst, int minimumSecond );
