...
private slots:
	...
	// STYLE-13's claim, checked as a claim -- and REVISED at the 2026-07-24 review. The band paints its own strong /
	// receded pair (band_surface), DECOUPLED from the selected tab: that is what lets the light strip's static field
	// match the receded band exactly while the unfocused active tab sits a step darker than both. What still holds --
	// and is asserted on rendered pixels, because left to the eye this class of claim was wrong for a whole round while
	// looking fine -- is that the band paints from the named rule in both states, and that the STRONG shades coincide:
	// a focused band and a focused active tab read as one register across the splitter.
	//
	// The reference is a ShadedTabBar rather than a Fusion bar, because that is what the application actually puts on
	// screen; comparing against Fusion would be comparing against a widget the product does not use.

	void the_pane_header_paints_the_band_rule ()
	{
		for ( const bool paneFocused : { false, true } )
		{
			...
			QCOMPARE ( bandFill, band_surface ( header.palette (), paneFocused ) );
			...
			// Focused: the band's strong shade and the focused ACTIVE tab's are the same colour.
			...
			reference.shaded_tab_bar ()->set_pane_focused ( true );
