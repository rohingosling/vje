...
private slots:
	...
	//=================================================================================================================
	// Keyboard resizing (NAV-06)
	//=================================================================================================================

	// The ordinary nudge: the first pane takes the delta and the second gives it up, so the pair still fills the same
	// total. A splitter that grew the window would be the alternative, and is not one.

	void a_nudge_moves_width_from_one_pane_to_the_other ()
	{
		const QList<int> moved = nudged_splitter_sizes ( { 300, 700 }, 16, 200, 200 );

		QCOMPARE ( moved, QList<int> ( { 316, 684 } ) );

		const QList<int> back = nudged_splitter_sizes ( { 300, 700 }, -16, 200, 200 );

		QCOMPARE ( back, QList<int> ( { 284, 716 } ) );
	}

	// At either end the nudge STOPS rather than squeezing, so holding the key parks the splitter at the edge instead of
	// walking the other pane out of existence.

	void a_nudge_stops_at_each_pane_minimum ()
	{
		const QList<int> atFirstMinimum = nudged_splitter_sizes ( { 210, 790 }, -100, 200, 200 );

		QCOMPARE ( atFirstMinimum, QList<int> ( { 200, 800 } ) );

		const QList<int> atSecondMinimum = nudged_splitter_sizes ( { 790, 210 }, 100, 200, 200 );

		QCOMPARE ( atSecondMinimum, QList<int> ( { 800, 200 } ) );

		// Already parked: another press changes nothing rather than overshooting into the other pane.

		QCOMPARE ( nudged_splitter_sizes ( { 200, 800 }, -16, 200, 200 ), QList<int> ( { 200, 800 } ) );
	}

	// A window too narrow to satisfy both minimums must still return something sane -- the first pane keeps its own
	// minimum, and nothing goes negative.

	void a_window_narrower_than_both_minimums_does_not_go_negative ()
	{
		const QList<int> squeezed = nudged_splitter_sizes ( { 150, 150 }, -16, 200, 200 );

		QCOMPARE ( squeezed.at ( 0 ), 200 );
		QVERIFY  ( squeezed.at ( 1 ) <= 100 );
		QCOMPARE ( squeezed.at ( 0 ) + squeezed.at ( 1 ), 300 );
	}

	// Anything that is not exactly two panes is returned untouched: this workspace has two, and guessing at a third
	// would be worse than doing nothing.

	void a_list_that_is_not_two_panes_is_left_alone ()
	{
		QCOMPARE ( nudged_splitter_sizes ( { 300 },           16, 200, 200 ), QList<int> ( { 300 } ) );
		QCOMPARE ( nudged_splitter_sizes ( { 300, 300, 300 }, 16, 200, 200 ), QList<int> ( { 300, 300, 300 } ) );
		QCOMPARE ( nudged_splitter_sizes ( {},                16, 200, 200 ), QList<int> () );
	}
};
